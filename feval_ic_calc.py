...
def process_single_factor(
    ...
    # 单因子回测
    print("单因子回测...")
    performance_result = backtest(portfolio_weights, vwap_df, rebalance_frequency=20)
...
def batch_factor_analysis(
    ...
    combined_results = optimize_ic_report(pd.concat(all_factor_results))
    ...
    report_path = f"{save_dir}/combined_factor_analysis_{timestamp}.csv"
